...
import {
  ...
  DEFAULT_LINEAR,
  type Gradient,
} from "@/registry/new-york/fill-picker-base/gradient";
  ...
  return (
    <main className="mx-auto max-w-3xl space-y-8 px-6 py-12">
      ...
      <section className="space-y-3">
        <h2 className="text-lg font-medium">Gradient picker</h2>
        <p className="text-muted-foreground">
          Only the parts that touch a Select or a plain button are rebuilt
          on Base UI (<code className="font-mono">TypeSwitcher</code>,{" "}
          <code className="font-mono">InterpSwitcher</code>,{" "}
          <code className="font-mono">RadialSizeSelect</code>,{" "}
          <code className="font-mono">ReverseStops</code>,{" "}
          <code className="font-mono">StopList</code>). Everything else —
          Bar, Area, Overlay, the angle/position pads, ShapeSwitcher,
          StopColor, Presets, CssInput — is pure pointer/canvas logic and is
          imported straight from the original gradient picker.
        </p>
      </section>

      <section className="rounded-lg border border-border bg-popover p-4 shadow-sm">
        <GradientPickerBase.Root value={gradient} onValueChange={setGradient}>
          <div className="space-y-3">
            <div className="flex items-center justify-between gap-2">
              <GradientPickerBase.TypeSwitcher />
              <GradientPickerBase.ReverseStops />
            </div>
            <GradientPickerBase.Bar />
            <GradientPickerBase.Area />
            <GradientPickerBase.ShapeSwitcher />
            <GradientPickerBase.RadialSizeSelect />
            <GradientPickerBase.StopList />
            <GradientPickerBase.InterpSwitcher />
          </div>

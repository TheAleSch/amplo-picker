...
// Both variants expose identical compound namespaces, so every live example
// renders whichever trio the route passes in. Radix's namespaces type the
// context; the Base UI objects are structurally compatible (same parts, same
// props — the same cast the playground and hero use), so the casts are safe.
type Pickers = {
  CP: typeof ColorPicker;
  GP: typeof GradientPicker;
  FP: typeof FillPicker;
};
const PickersCtx = React.createContext<Pickers | null>(null);

function usePickers(): Pickers {
  const p = React.useContext(PickersCtx);
  if (!p) throw new Error("Docs examples must render inside <FullDocs>");
  return p;
}
...
function GradientShapeControls() {
  ...
  if (ctx.gradient.type === "linear") {
    return (
      <GP.AngleGroup>
        <GP.AnglePad />
        <GP.AngleInput className="flex-1" />
      </GP.AngleGroup>
    );
  }
...
export function FullDocs({ variant }: { variant: Variant }) {
  ...
  const pickers = React.useMemo(() => ({ CP, GP, FP }), [CP, GP, FP]);

  const adapt = React.useCallback(
    (s: string) =>
      variant === "base"
        ? SNIPPET_PATH_SWAPS.reduce((acc, [from, to]) => acc.replaceAll(from, to), s)
        : s,
    [variant],
  );

  return (
    <PickersCtx.Provider value={pickers}>
      ...
      <article className="flex max-w-3xl flex-col gap-8">
        ...
        <section className="flex flex-col gap-4">
          ...
          <p className="text-sm text-muted-foreground">
            ...
            code snippet on this page follows the active variant.
          </p>
        </section>
        ...
        <section className="flex flex-col gap-6">
          ...
          <Example
            ...
            code={adapt(GRADIENT_COMPACT_CODE)}
          />
          ...
          <Example
            ...
            code={adapt(GRADIENT_BAR_ONLY_CODE)}
          />
        ...
        <section className="flex flex-col gap-4">
          ...
          <Example
            ...
            code={adapt(GRADIENT_OVERLAY_CODE)}
          />
        ...
        <section className="flex flex-col gap-4">
          ...
          <p className="text-sm text-muted-foreground">
            ...
            <Code>
              {'<GP.Root defaultStopColorFormat="hex">'}
            </Code>
            .
          </p>
          <p className="text-sm text-muted-foreground">
        ...
        <section className="flex flex-col gap-4">
          ...
          <CodeBlock code={adapt(GRADIENT_OUTPUT_TYPE_CODE)} />
          <p className="text-sm text-muted-foreground">
          ...
          <CodeBlock code={adapt(GRADIENT_OUTPUT_CSS_CODE)} />
          <p className="text-sm text-muted-foreground">
        ...
        <section className="flex flex-col gap-6">
          ...
          <Example
            ...
            code={adapt(FILL_PICKER_TABS_CODE)}
          />
        </section>
        ...
        <section className="flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-4">
            <H3>Linear</H3>
            <CodeBlock code={adapt(GRADIENT_SHAPE_LINEAR_CODE)} />
            <H3>Radial · Circle</H3>
            <CodeBlock code={adapt(GRADIENT_SHAPE_RADIAL_CIRCLE_CODE)} />
            <H3>Radial · Ellipse</H3>
            <CodeBlock code={adapt(GRADIENT_SHAPE_RADIAL_ELLIPSE_CODE)} />
          </div>
        </section>
        ...
        <section className="flex flex-col gap-4">
          ...
          <CodeBlock code={adapt(ANATOMY_CODE)} />
        </section>
        ...
        <section className="flex flex-col gap-4">
          ...
          <p className="text-sm text-muted-foreground">
            ...
            <Code>{'<FP.Pane mode="gradient">'}</Code>) — they
            throw otherwise.
          </p>
        ...
        <section className="flex flex-col gap-4">
          ...
          <CodeBlock code={adapt(HOOK_CODE)} />
        </section>
...
function GradientCompactDemo() {
  const { CP, GP } = usePickers();
  const [g, setG] = React.useState<Gradient>(DEFAULT_LINEAR);
  return (
    <div className="w-full max-w-xs">
      <GP.Root value={g} onValueChange={setG}>
        <GP.Bar />
        <GP.TypeSwitcher />
        <GP.StopColor>
          <CP.Hue />
...
function FillPickerTabsDemo() {
  const { CP, GP, FP } = usePickers();
  const [fill, setFill] = React.useState<Fill>(() => ({
    kind: "color",
    color: parseColor("oklch(0.7 0.18 30)")!,
  }));
  return (
    <FP.Root value={fill} onValueChange={setFill} className="w-full max-w-xs">

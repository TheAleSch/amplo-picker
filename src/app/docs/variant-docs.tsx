...
// Both variants expose the identical compound namespace, so the live examples
// render whichever one the route passes in. Radix's namespace types the
// context; the Base UI object is structurally compatible (verified: same
// parts, same props), so the cast is safe.
type PickerNs = typeof ColorPicker;
const PickerCtx = React.createContext<PickerNs | null>(null);

function useCP(): PickerNs {
  const cp = React.useContext(PickerCtx);
  if (!cp) throw new Error("Color-picker examples must render inside VariantDocs");
  return cp;
}

const RADIX_PATH = "@/components/ui/color-picker/color-picker";
const BASE_PATH = "@/components/ui/fill-picker-base/color-picker";
...
export function VariantDocs({ variant }: { variant: Variant }) {
  const CP = (variant === "base"
    ? (ColorPickerBase as unknown as PickerNs)
    : ColorPicker) as PickerNs;

  // Code snippets are authored against the Radix import path; for Base UI we
  // swap only the path (the exported `ColorPicker` name is identical thanks to
  // the alias in the Base UI barrel).
  const adapt = React.useCallback(
    (s: string) =>
      variant === "base" ? s.replaceAll(RADIX_PATH, BASE_PATH) : s,
    [variant],
  );

  return (
    <PickerCtx.Provider value={CP}>
      <main className="mx-auto grid min-h-screen max-w-6xl grid-cols-1 gap-12 px-6 py-16 lg:grid-cols-[1fr_220px]">
        <article className="flex max-w-3xl flex-col gap-8">
          ...
          <PreviewTabs preview={<HeroExample />} code={adapt(HERO_CODE)} />
            ...
            {variant === "base" ? (
              <>
                <InstallTabs
                  ...
                  description="The default variant, built on Base UI primitives (Slider, Select, NumberField, RadioGroup). Drops the picker into components/ui/fill-picker-base/ and pulls the shared engine in as a registry dependency."
                />
                <p className="text-sm text-muted-foreground">
                  Prefer Radix-backed shadcn parts? Switch to the{" "}
                  <Link href="/docs/radix" className="underline">
                    Radix variant
                  </Link>{" "}
                  — same compound API and OKLCH engine, so behavior and fixes
                  stay in lockstep.
                </p>
              </>
            ) : (
              <>
                ...
                <p className="text-sm text-muted-foreground">
                  ...
                  is the default going forward — same compound API and OKLCH
                  engine, so behavior and fixes stay in lockstep.
                </p>
              </>
            )}
          </section>
          ...
          <section className="flex flex-col gap-4">
            ...
            <p>
              ...
              <Code>formats</Code> record, so a fallback (e.g. hex) is always
              one property access away.
            </p>
            <CodeBlock code={adapt(USAGE_CODE)} />
          </section>
          ...
          <section className="flex flex-col gap-6">
            ...
            <Example
              ...
              code={adapt(SAVED_SWATCHES_CODE)}
            />
          </section>
          ...
          <section className="flex flex-col gap-4">
            ...
            <p>
              Headless layer powering every part. Use it directly when you want
              a totally custom UI but the same state machine.
            </p>
          ...
          <section className="flex flex-col gap-4">
            ...
            <H3>sRGB</H3>
            <p>
              The baseline web gamut. Every device made in the last 30 years
              can render it. <Code>#hex</Code>, <Code>rgb()</Code>,{" "}
              <Code>hsl()</Code> all live here.
            </p>
            <H3>Display-P3</H3>
            <p>
              Apple&apos;s wide-gamut space. Every recent iPhone, iPad, and
              MacBook supports it; many newer Android phones too. About 25%
              wider than sRGB, especially in reds and greens. Authored as{" "}
            ...
            <p className="text-sm text-muted-foreground">
              ...
              with in-gamut color for the active render gamut; warning lines
              mark the cutoffs of narrower gamuts <em>inside</em> the fill. Pick{" "}
              <Code>hex</Code>/<Code>rgb</Code>/<Code>hsl</Code>/<Code>hsb</Code>{" "}
              and the area fills with sRGB only — no warning line. Pick{" "}
              <Code>p3</Code> and a single thin line marks the sRGB cutoff
          ...
          <section className="flex flex-col gap-4">
            ...
            <ul className="flex list-disc flex-col gap-1.5 pl-5">
              <li>
                ...
                <Code>aria-roledescription</Code> and{" "}
                <Code>aria-valuetext</Code> describing the current point.
              </li>
              <li>
              ...
              <li>
                <strong>Reduced motion.</strong> Transitions are gated behind{" "}
                <Code>motion-safe:</Code>, so they respect the user&apos;s
    ...
    <div className="flex w-full flex-col gap-3 sm:flex-row">
      ...
      <div className="flex flex-1 flex-col gap-2">
        ...
        <CP.Root
          ...
          defaultFormat="oklch"
        >
          <CP.Area gamut="rec2020" softProof />
          <CP.Hue />
        </CP.Root>
      </div>
    </div>
